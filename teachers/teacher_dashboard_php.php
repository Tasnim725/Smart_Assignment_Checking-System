...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create_assignment'])) {
    if (verify_csrf_token($_POST['csrf_token'])) {
        ...
        if ($stmt->execute()) { $stmt->close(); header("Location: teacher_dashboard.php?success=created"); exit(); }
        $stmt->close();
    }
}

// Stats
$stmt = $conn->prepare("SELECT COUNT(DISTINCT a.id) as total_assignments, COUNT(s.id) as total_submissions, COUNT(CASE WHEN s.evaluation_status='evaluated' THEN 1 END) as evaluated, AVG(CASE WHEN s.overall_score IS NOT NULL THEN s.overall_score END) as avg_score FROM assignments a LEFT JOIN submissions s ON a.id = s.assignment_id WHERE a.teacher_id = ?");
$stmt->bind_param("s", $teacher_id); $stmt->execute(); $stats = $stmt->get_result()->fetch_assoc(); $stmt->close();

// Recent submissions
$stmt = $conn->prepare("SELECT s.*, a.title as assignment_title, a.batch_number FROM submissions s JOIN assignments a ON s.assignment_id = a.id WHERE a.teacher_id = ? ORDER BY s.submitted_at DESC LIMIT 10");
$stmt->bind_param("s", $teacher_id); $stmt->execute(); $submissions = $stmt->get_result(); $stmt->close();
...
<div class="container-fluid py-4">
    <div class="row mb-4"><div class="col"><h2>Welcome, <?php echo htmlspecialchars($_SESSION['user_name'], ENT_QUOTES, 'UTF-8'); ?>! 👨‍🏫</h2></div></div>
    ...
    <div class="row">
        ...
        <div class="col-lg-5 mb-4">
            <div class="card">
                <div class="card-header"><i class="bi bi-plus-circle me-2"></i>Create New Assignment</div>
                <div class="card-body">
                    <form method="POST">
            ...
            <div class="card">
                ...
                <div class="card-body p-0">
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="evaluate.php?id=<?php echo $sub['id']; ?>" class="btn btn-sm btn-success"><i class="bi bi-robot me-1"></i>Evaluate</a>
                                    <?php else: ?>
                                    <a href="../shared/view_details.php?id=<?php echo $sub['id']; ?>" class="btn btn-sm btn-primary"><i class="bi bi-eye me-1"></i>View</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                    ...
                    <div class="card-footer bg-white text-center">
                        <a href="teacher_submissions.php" class="btn btn-outline-primary btn-sm">View All <i class="bi bi-arrow-right ms-1"></i></a>
                    </div>
                    <?php else: ?>
